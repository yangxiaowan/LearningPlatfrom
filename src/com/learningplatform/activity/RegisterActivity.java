...
public class RegisterActivity extends Activity {
		...
		mRegisterButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (mUserNameState == RegisterInfoCheckState.REGISTER_USERNAME_NULL) {
					...
					Toast.makeText(getApplicationContext(), "输入用户名格式不正确",
							Toast.LENGTH_SHORT).show();
					return;
				}
				if (mPasswordFristState == RegisterInfoCheckState.REGISTER_PASSWORDFRIST_NULL) {
					...
					Toast.makeText(getApplicationContext(), "输入密码格式不正确",
							Toast.LENGTH_SHORT).show();
					return;
				}
				...
				if (mEmailState == RegisterInfoCheckState.REGISTER_USEREMAIL_NULL) {
					...
					Toast.makeText(getApplicationContext(), "输入邮箱名格式不正确",
							Toast.LENGTH_SHORT).show();
					return;
				}
				if (mVertificationState == RegisterInfoCheckState.REGISTER_VERTIFICATION_NULL) {
					...
					Toast.makeText(getApplicationContext(), "输入验证码不正确",
							Toast.LENGTH_SHORT).show();
					return;
				}
				new AsyncTask<String, Float, Boolean>() {
					...
					protected Boolean doInBackground(String... params) {
						try {
							...
							try {
								...
								connection.setRequestProperty("user_name",
										params[1]);
								connection.setRequestProperty("user_password",
										params[2]);
								connection.setRequestProperty("user_email",
										params[3]);
								connection.connect();
								...
								if (responseCode == 200) {
									...
									Log.i("register_servlet", "注册结果码为： "
											+ mRegisterResult);
									dis.close();
									is.close();
									return true;
								} else {
									Log.i("register_servlet", "链接服务器失败"
											+ responseCode);
									return false;
								}
							} catch (IOException e) {
					 ...
					 * //注册可以通过了，但未成功 public static final int REGISTER_FAILED =
					 * 0; //注册失败了 public static final int REGISTER_SUCCESSFULLY
					 * = 2; //注册成功了 public static final int USER_ACCOUNT_EXSIT =
					 * 3; //注册用户账号存在 public static final int USER_EMAIL_EXSIT =
					 * 4; //注册邮箱存在
					 */
					protected void onPostExecute(Boolean result) {
						if (mRegisterResult == 2) {
							Toast.makeText(getApplicationContext(), "恭喜账号注册成功",
							...
							Toast.makeText(getApplicationContext(), "改用户名已被注册",
									Toast.LENGTH_SHORT).show();
						} else if (mRegisterResult == 4) {
							Toast.makeText(getApplicationContext(), "该邮箱已经被注册",
									Toast.LENGTH_SHORT).show();
						} else {
							Toast.makeText(getApplicationContext(),
									"不知道出了什么问题，万君也无能为力", Toast.LENGTH_SHORT)
									.show();
						}
					};
				}.execute(ServerServletURL.REGISTER_SERVLET_LOCATION, userName,
						userPasswordFirst, userEmail);
			}
		});
	}
	...
	public void initView() { // 初始化视图
		...
		mCodeImage = (ImageView) this
				.findViewById(R.id.register_vertificationcode_imageview);
		mCodeText = (TextView) this
				.findViewById(R.id.register_changecode_textview);
		mRegisterName = (EditText) this
				.findViewById(R.id.register_username_textview);
		mRegisterPasswordFrist = (EditText) this
				.findViewById(R.id.register_passwordfirst_textview);
		mRegisterPasswordSecond = (EditText) this
				.findViewById(R.id.register_passwordsecond_textview);
		mCodeInput = (EditText) this
				.findViewById(R.id.register_inputcode_edittext);
		mRegisterEmail = (EditText) this
				.findViewById(R.id.register_email_textview);
		...
		mCodeText.setOnClickListener(new TextView.OnClickListener() { // 点击更换一张验证码后，随机产生一张验证码
					@Override
					public void onClick(View view) {
						VerificationCode mBitmapCode = VerificationCode
								.getInstance();
						mCodeImage.setImageBitmap(mBitmapCode
								.createCodeBitmap());
						codeString = mBitmapCode.getRandomCode();
					}
				});
